...
def reduce_dimension_ndc(df, ndc_df):
    ...
    '''
    df_merged = df.merge(ndc_df[['NDC_Code', 'Non-proprietary Name']], how='left', left_on='ndc_code', right_on='NDC_Code')
    df_merged = df_merged.rename(columns={'Non-proprietary Name': 'generic_drug_name'})
    df_merged = df_merged.drop('NDC_Code', axis=1)
...
def patient_dataset_splitter(df, patient_key='patient_nbr'):
    ...
    test_size = round(total_values * (0.2))
    train = df[df[patient_key].isin(unique_values[:train_size])].reset_index(drop=True)
    validation = df[df[patient_key].isin(unique_values[train_size: train_size + test_size])].reset_index(drop=True)
    test = df[df[patient_key].isin(unique_values[train_size + test_size:])].reset_index(drop=True)
...
def create_tf_categorical_feature_cols(categorical_col_list,
    ...
    for c in categorical_col_list:
        ...
        if dim_count > 10:
            dims = 10
            tf_categorical_feature = tf.feature_column.embedding_column(tf_categorical_feature_column, dimension=dims)        
        else:
            tf_categorical_feature = tf.feature_column.indicator_column(tf_categorical_feature_column) 
       
        output_tf_list.append(tf_categorical_feature)
